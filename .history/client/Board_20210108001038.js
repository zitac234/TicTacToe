...
export default class Board extends React.Component {
  ...
  ComputerPlaceMark() {
    console.log("inside computerplacemark loop1");
    const index = this.refs.child.ComputerPlaceMark()
    const [r, c] = [this.[0], this.findBestMove()[1]];
    this.state.grid.removeEventListener("click", () => {
      this.state.grid.rows[r].cells[c].innerHTML = this.state.Computermark;
      console.log("inside computerplacemark loop2");
    });
  }
  placeMark() {
    ...
    table.addEventListener("click", (event) => {
      ...
      if (mark === this.state.ComputerMark) {
