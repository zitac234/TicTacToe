...
export default class Board extends React.Component {
  constructor(props) {
    ...
    this.disappearComponent = this.disappearComponent.bind(this);
  }
  componentDidMount() {
    document.addEventListener("click", this.disappearComponent);
    this.placeMark();
  }
  componentWillUnmount(){
      document.removeEventListener('click', this.disappearComponent)
  }
  disappearComponent(){
      document.removeEventListener('click', this.di)
  }
  rowWin(mark) {
    for (let i = 0; i < 3; i++) {
      for (let n = 0; n < 3; n++) {
