...
export default class Board extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      mycell: document.querySelectorAll("td"),
      grid: table,
      PlayerMark: "O",
      ComputerMark: "X",
      mounted: false
  ...
  //if index is passed, see if cell is empty,
  //if not, see if any cells are empty
  emptyCell(index) {
    let cell;
    let table = this.state.grid;// 
    if (index) { //index = [row, column] of cell, if cell is passed
      ...
      for (let i = 0; i < 3; i++) {
        for (let n = 0; n < 3; n++) {
          ...
          if (cell.innerHTML === "") { // if one cell is empty, return true
            return true;
          }
  ...
  rowWin(mark) { //checking for 3 of same mark in a row
    for (let i = 0; i < 3; i++) {
      for (let n = 0; n < 3; n++) {
  ...
  colWin(mark) {  //checking for 3 of same mark in a col
    for (let i = 0; i < 3; i++) {
      for (let n = 0; n < 3; n++) {
  ...
  diagWin(mark) {  //checking for 3 of same mark in a diagonal
    let n = 2;
    for (let i = 0; i < 3; i++) {
